...
\newcommand{\x}[1]{{\sf #1}}
\newcommand{\seal}{\emph{SEAL}}
\newcommand{\prism}{\emph{PRISM}}
...
\mathit{rules} &::=& \mathit{condition} [ \x{action}\textrm{-}\x{label}]\Rightarrow \mathit{distribution} \Sep \mathit{rules},\mathit{rule}
	\\[.3cm]
\mathit{condition} &::=& \x{E} \Space \x{id}:\x{module}\textrm{-}\x{name} \Space . \Space \mathit{condition} \Sep \mathit{expression} \Join \mathit{expression} \\
