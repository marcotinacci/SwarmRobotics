...
class FormulaHandler {
	...
	new(Resource resource) {
		...
		if(p.isEmptyEnv)
			...
			totalParamsNumber = p.environment.countParams + subjectParamsNumber
		index = newHashMap
	}
	...
	def execModelCheck(String formula) { 
		// TODO get the formula directly form resource
	...
	def dispatch Integer countParams(Parallel par) { 
		par.left.countParams + par.right.countParams
	}
	
	// TODO fattorizzare in un pacchetto utils
	def dispatch Integer countParams(SingleModule ref){
		ref.module.variables.size
	}
}